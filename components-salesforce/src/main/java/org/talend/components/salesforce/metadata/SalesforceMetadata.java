...
//        mapping.put(FieldType.location, SF_Location.class);
    ...
    private Schema getSchema(SalesforceModuleProperties props) throws TalendConnectionException {
        ...
        try {
            connection = connManager.newConnection(props.getConnection());//TODO: replace to connection pool after ConnectionManager implement getConnectionPoolByKey
            DescribeSObjectResult describeSObjectResult = connection.getPartnerConnection().describeSObject(props.moduleName.getStringValue());
            Field[] fields = describeSObjectResult.getFields();
            for (Field field : fields) {
                root.addChild(SchemaFactory.newDataSchemaElement(SalesforceBaseType.FAMILY_NAME, field.getName(), mapping.get(field.getType())));
            }
    ...
    public void initSchemaForDynamic(ComponentProperties properties) throws TalendConnectionException {
        SalesforceModuleProperties props = (SalesforceModuleProperties) properties;
        Schema schema = (Schema) (props.schema.schema.getValue());
        List<SchemaElement> children = schema.getRoot().getChildren();
        SchemaElement dynamicElement = null;
        List<String> unDynamicElementsName = new ArrayList<>();
        for (SchemaElement child : children) {
            if (child.getType() == SchemaElement.Type.DYNAMIC) {
        ...
        if (dynamicElement != null) {
            Schema realSchema = getSchema(props);
            List<SchemaElement> realElements = realSchema.getRoot().getChildren();
            for (SchemaElement realElement : realElements) {
