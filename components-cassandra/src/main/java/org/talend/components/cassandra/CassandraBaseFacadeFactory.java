...
import org.talend.daikon.schema.avro.util.HasNestedAvroConverter;
 ...
 * 
 * This is a good example of how to construct an {@link IndexedRecordFacadeFactory} that supports all Avro collection
 ...
 */
public abstract class CassandraBaseFacadeFactory<GettableT extends GettableByIndexData, SettableT extends SettableByIndexData<SettableT>, TypeT>
        implements IndexedRecordFacadeFactory<GettableT, IndexedRecord>, HasNestedAvroConverter<GettableT, IndexedRecord> {
    ...
    public Iterable<AvroConverter<?, ?>> getNestedAvroConverters() {
        for (Schema.Field f : AvroUtils.unwrapIfNullable(getSchema()).getFields()) {
            ...
            if (mFieldConverter[i] == null) {
                mFieldConverter[i] = (AvroConverter<Object, Object>) CassandraAvroRegistry.get().getConverter(mFieldType[i],
                        f.schema(), null);
            }
        }
        return Arrays.asList(mFieldConverter);
    }
    ...
    public IndexedRecord convertToAvro(GettableT gettable) {
        ...
        GettableFacadeIndexedRecord record = new GettableFacadeIndexedRecord(gettable);
    ...
    public class GettableFacadeIndexedRecord implements IndexedRecord, Comparable<IndexedRecord> {
        ...
        public GettableFacadeIndexedRecord(GettableT gettable) {
            mGettable = gettable;
        }
        ...
        public Object get(int i) {
            ...
            if (mFieldConverter[i] == null) {
                mFieldConverter[i] = CassandraAvroRegistry.get().getConverter(mFieldType[i],
                        AvroUtils.unwrapIfNullable(mSchema).getFields().get(i).schema(), value.getClass());
            }
